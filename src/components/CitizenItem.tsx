...
const CitizenItem: React.FC<CitizenType> = ({ id, age, city, name, someNote }) => {
  ...
  return (
    <Container>
      ...
      <p>Age: {age}</p>
      <p>City: {city}</p>
      <CitizenNote>{name}'s note: {someNote}</CitizenNote>
      <Button onClick={onShowHandler}>{note ? 'Hide' : 'See more'}</Button>
    </Container>
  );
};
